...
import { runMonolithCompile } from '../_shared/compiler/monolith.ts'
...
serve(async (req) => {
  ...
  try {
    ...
    if (useStream) {
      ...
      const stream = new ReadableStream({
        async start(controller) {
          ...
          try {
            const result = await runMonolithCompile(emit, body, authHeader)
    ...
    const result = await runMonolithCompile(() => {}, body, authHeader)
    ...
    console.error('i420-compile-single error:', error)
    return new Response(
