/**
 * Structured runtime error hints for agent flow tool nodes.
 * Used by execute-node and surfaced in Runtime UI / future Design chat feedback.
 */
...
const ERROR_CATALOG: Record<string, Omit<RuntimeErrorInfo, 'error_code'>> = {
  ...
  channel_not_found: {
    ...
    hint: 'Check the channel ID or name in the slack_fetch_messages / slack_notify node config.',
  },
  missing_scope: {
  ...
  slack_not_connected: {
    ...
    hint: 'Connect Slack in Admin → Integrations Hub before running this flow.',
  },
}
