...
export async function resolveCompilerConfig(
  supabase: ReturnType<typeof createClient>,
...
export type MonolithCompileBody = { prompt: string; agent_id: string; action?: string }

// ── Main compile (single-stage monolith) ─────────────────────────────────────
export async function runMonolithCompile(
  emit: StatusEmitter,
  body: MonolithCompileBody,
  authHeader: string,
) {
    if (!authHeader) {
    ...
    const { data: newVersion, error: versionError } = await supabase
      ...
      .insert({
        ...
        version: nextVersion,
        flow_json: flowJSON,
        compile_artifacts: sessionArtifacts,
    ...
    return {
      ...
      chat_history: newHistory.slice(-50),
      compiler: { provider: compilerConfig.provider, model: compilerConfig.model, mode: 'single' },
    }
}
